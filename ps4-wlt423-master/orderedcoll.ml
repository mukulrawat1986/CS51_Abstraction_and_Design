...
module BinSTree (C : COMPARABLE)
  ...
  struct
    ...
    let getmin (t : tree) : elt =
      let rec rgetmin ( t : tree) : elt = 
        match t with
        | Leaf -> raise EmptyTree
        | Branch (Leaf, lst, _) ->
          match lst with
            | [lst] -> lst
            | _ :: tl -> rgetmin Branch (Leaf, lst, Leaf)
        | Branch (l, _, _) -> rgetmin l
;   (*..................................................................
    ...
    let rec getmax (t : tree) : elt =
        ...
        | Branch (_, lst, Leaf) ->
          match lst with
          | [lst] -> lst
          | _ :: tl -> rgetmax (Branch (Leaf, lst, Leaf))
        | Branch (_, _, r) -> rgetmax r
